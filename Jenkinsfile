                ...
                stage('Compile & Verify') {
                    steps {
                        script {
                            ...
                            try {
                                sh '''
                                mvn --batch-mode -q clean verify -DskipTests=false -Dtest.log.level=DEBUG | grep -E "Tests run:|Failures:|Errors:|Skipped:|DEBUG"
                                '''
                                logSuccess("BUILD", "Compilation and tests completed successfully.")
                            } catch (Exception e) {
                                logFailure("BUILD", "Build failed: ${e.message}")
                ...
                stage('Static Code Analysis') {
                    steps {
                        withCredentials([string(credentialsId: 'sonar-token', variable: 'SONAR_TOKEN')]) {
                            script {
                                logInfo("ANALYSIS", "Running SonarQube analysis...")
                                try {
                                    sh '''
                                    mvn --batch-mode -q sonar:sonar \
